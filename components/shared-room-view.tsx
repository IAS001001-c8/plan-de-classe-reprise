...
export function SharedRoomView({ roomAssignment }: SharedRoomViewProps) {
  const room = roomAssignment.rooms
  const seatAssignments = roomAssignment.seat_assignments || {}
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription className="text-lg mt-2">Classe: {roomAssignment.class_name}</CardDescription>
              </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {room.config.columns.map((column: any, colIndex: number) => (
