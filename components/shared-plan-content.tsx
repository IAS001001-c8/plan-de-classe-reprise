...
interface SeatAssignment {
  studentName: string
  placedBy: "admin" | "student" // admin = délégué/prof/vie scolaire, student = élève
}
...
export function SharedPlanContent() {
  ...
  useEffect(() => {
    if (nameConfirmed && studentName && seatAssignments) {
      // Chercher si l'élève a déjà une place
      const studentSeat = Object.entries(seatAssignments).find(
        ([_, assignment]) => assignment.studentName.toLowerCase() === studentName.toLowerCase(),
      )
  ...
  }, [nameConfirmed, studentName, seatAssignments])
  ...
  const handleSeatSelect = (seatId: string) => {
    if (!shareData || !nameConfirmed) return
    ...
    if (seatAssignments[seatId]) {
      ...
      if (seatAssignments[seatId].studentName.toLowerCase() === studentName.toLowerCase()) {
      ...
      toast({
        title: "Place occupée",
        description: `Cette place est actuellement occupée par ${seatAssignments[seatId].studentName}. Votre sélection remplacera cet élève.`,
      })
    }
  ...
  const saveAssignments = (assignments: Record<string, SeatAssignment>) => {
    if (!shareData) return
        ...
        <CardContent>
          <div className="space-y-6">
            <Alert>
              ...
              <AlertDescription>
                Bonjour {studentName}. Sélectionnez une place disponible dans la salle.
                {selectedSeat && <div className="mt-2 font-semibold">Votre place actuelle: {selectedSeat}</div>}
              </AlertDescription>
            </Alert>
                        ...
                        <div className="flex justify-between w-full">
                          {Array.from({ length: column.seatsPerTable }).map((_, seatIndex) => {
                            ...
                            const isOccupiedByCurrentStudent =
                              isOccupied && assignment.studentName.toLowerCase() === studentName.toLowerCase()
                            const isOccupiedByOtherStudent =
                              isOccupied &&
                              assignment.studentName.toLowerCase() !== studentName.toLowerCase() &&
                                ...
                                >
                                  {isOccupied ? (
                                    <div className="flex flex-col items-center">
                                      ...
                                      <span className="text-xs">{assignment.studentName.split(" ")[1]?.[0] || ""}</span>
                                      {isOccupiedByAdmin && (
